...
class PDF_Invoice extends FPDF
{
	...
	function addAlmacen($almacen)
	{
		...
		$y2		= 30;
		$mid = $y1 + ($y2 / 2);
		$this->RoundedRect($r1, $y1 - 10, ($r2 - $r1 + 55), $y2 - 10, 3.5, 'D');
		$this->Line($r1, $mid - 17, $r2 + 55, $mid - 17);
		$this->SetXY($r1 - 10 + ($r2 - $r1 + 75) / 2 - 5, $y1 - 8);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 5, utf8_decode("Ubicación del almacén: "), 0, 0, "C");
		$this->SetXY($r1, $y1);
		$this->SetFont("Arial", "B", 10);
		$this->MultiCell(60, 4, utf8_decode(""));
		$this->SetXY($r1 + 2, $y1 + 2);
		$this->SetFont("Arial", "", 10);
		$this->MultiCell(150, 4, $almacen);
	}
	...
	function addZona($zona)
	{
		...
		$y2		= 30;
		$mid = $y1 + ($y2 / 2);
		$this->RoundedRect($r1 + 97, $y1 - 10, ($r2 - $r1 + 55), $y2 - 10, 3.5, 'D');
		$this->Line($r1 + 97, $mid - 17, $r2 + 152, $mid - 17);
		$this->SetXY($r1 + 88 + ($r2 - $r1 + 75) / 2 - 5, $y1 - 8);
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 5, utf8_decode("Zona de entrega: "), 0, 0, "C");
		$this->SetXY($r1, $y1);
		$this->SetFont("Arial", "B", 10);
		$this->MultiCell(60, 4, utf8_decode(""));
		$this->SetXY($r1 + 99, $y1 + 2);
		$this->SetFont("Arial", "", 10);
		$this->MultiCell(150, 4, $zona);
	}
