...
if (!isset($_SESSION["nombre"])) {
  ...
  if ($_SESSION['solicitud'] == 1) {
    ...
    $pdf->Cell(40, 6, '', 0, 0, 'C');
    $pdf->Cell(100, 6, 'LISTA DE SOLICITUDES', 1, 0, 'C');
    $pdf->Ln(10);
    ...
    $pdf->Cell(40, 6, 'Fecha despacho', 1, 0, 'C', 1);
    ...
    while ($reg = $rspta->fetch_object()) {
      ...
      $fecha_hora_despacho = ($reg->fecha_hora_despacho == "01-01-2000 00:00:00") ? "Sin registrar" : $reg->fecha_hora_despacho;
      $responsable_pedido = ucwords($reg->responsable_pedido);
      $responsable_despacho = ($reg->idalmacenero == 0 || $reg->idalmacenero == "0") ? "Sin registrar" : ucwords($reg->responsable_despacho);
      $codigo_pedido = 'N° ' . $reg->codigo_pedido;

      $pdf->SetFont('Arial', '', 10);
      $pdf->Row(array(utf8_decode($fecha_hora_pedido), utf8_decode($fecha_hora_despacho), utf8_decode($responsable_pedido), utf8_decode($responsable_despacho), utf8_decode($codigo_pedido), utf8_decode($numero)));
    }
