...
class Transferencia
{
	...
	public function agregar($origen, $destino, $idusuario, $codigo, $lugar_destino, $comentario, $idarticulo, $cantidad)
	{
		...
		while ($num_elementos < count($idarticulo)) {
			...
			if ($origen != $destino) {
				...
				$sql_actualizar_stock_origen = "UPDATE articulo SET stock = stock - '$cantidad[$num_elementos]' WHERE idarticulo = '$idarticulo[$num_elementos]' AND idalmacen = '$origen'";
				...
				if ($articulo_datos) {
					...
					$sql_insertar_articulo_destino = "INSERT INTO articulo 
													  (idusuario, idcategoria, idalmacen, idmarca, idmedida, codigo, codigo_producto, nombre, stock, stock_minimo, precio_compra, precio_venta, 
													   precio_venta_mayor, ganancia, descripcion, talla, color, peso, imagen, comision, nota_1, nota_2, fecha_emision, fecha_vencimiento, estado, eliminado) 
													  VALUES 
													  ('" . $articulo_datos['idusuario'] . "', 
													   ...
													   '" . $articulo_datos['precio_venta'] . "', 
													   '" . $articulo_datos['precio_venta_mayor'] . "', 
													   '" . $articulo_datos['ganancia'] . "', 
													   '" . $articulo_datos['descripcion'] . "', 
													   '" . $articulo_datos['talla'] . "', 
													   '" . $articulo_datos['color'] . "', 
													   '" . $articulo_datos['peso'] . "', 
													   '" . $articulo_datos['imagen'] . "', 
													   '" . $articulo_datos['comision'] . "', 
	...
	{
		$sql = "SELECT tr.idtransferencia,tr.idusuario,u.nombre as usuario,u.cargo as cargo,u.cargo,loo.titulo as origen,lod.titulo as destino,tr.lugar_destino,tr.codigo,tr.comentario,DATE_FORMAT(tr.fecha_hora, '%d-%m-%Y %H:%i:%s') as fecha,tr.estado,SUM(dtr.cantidad) as total_cantidad FROM transferencias tr LEFT JOIN almacen loo ON tr.origen = loo.idalmacen LEFT JOIN almacen lod ON tr.destino = lod.idalmacen LEFT JOIN detalle_transferencia dtr ON tr.idtransferencia = dtr.idtransferencia LEFT JOIN usuario u ON tr.idusuario=u.idusuario GROUP BY tr.idtransferencia ORDER BY tr.idtransferencia DESC";
		return ejecutarConsulta($sql);
	}

	public function listarPorFecha($fecha_inicio, $fecha_fin)
	{
		$sql = "SELECT tr.idtransferencia,tr.idusuario,u.nombre as usuario,u.cargo as cargo,u.cargo,loo.titulo as origen,lod.titulo as destino,tr.lugar_destino,tr.codigo,tr.comentario,DATE_FORMAT(tr.fecha_hora, '%d-%m-%Y %H:%i:%s') as fecha,tr.estado,SUM(dtr.cantidad) as total_cantidad FROM transferencias tr LEFT JOIN almacen loo ON tr.origen = loo.idalmacen LEFT JOIN almacen lod ON tr.destino = lod.idalmacen LEFT JOIN detalle_transferencia dtr ON tr.idtransferencia = dtr.idtransferencia LEFT JOIN usuario u ON tr.idusuario=u.idusuario WHERE DATE(tr.fecha_hora) >= '$fecha_inicio' AND DATE(tr.fecha_hora) <= '$fecha_fin' GROUP BY tr.idtransferencia ORDER BY tr.idtransferencia DESC";
		return ejecutarConsulta($sql);
	}

	public function listarPorUsuario($idalmacenSession)
	{
		$sql = "SELECT tr.idtransferencia,tr.idusuario,u.nombre as usuario,u.cargo as cargo,u.cargo,loo.titulo as origen,lod.titulo as destino,tr.lugar_destino,tr.codigo,tr.comentario,DATE_FORMAT(tr.fecha_hora, '%d-%m-%Y %H:%i:%s') as fecha,tr.estado,SUM(dtr.cantidad) as total_cantidad FROM transferencias tr LEFT JOIN almacen loo ON tr.origen = loo.idalmacen LEFT JOIN almacen lod ON tr.destino = lod.idalmacen LEFT JOIN detalle_transferencia dtr ON tr.idtransferencia = dtr.idtransferencia LEFT JOIN usuario u ON tr.idusuario=u.idusuario WHERE tr.origen = '$idalmacenSession' GROUP BY tr.idtransferencia ORDER BY tr.idtransferencia DESC";
		return ejecutarConsulta($sql);
	}

	public function listarPorUsuarioFecha($idalmacenSession, $fecha_inicio, $fecha_fin)
	{
		$sql = "SELECT tr.idtransferencia,tr.idusuario,u.nombre as usuario,u.cargo as cargo,u.cargo,loo.titulo as origen,lod.titulo as destino,tr.lugar_destino,tr.codigo,tr.comentario,DATE_FORMAT(tr.fecha_hora, '%d-%m-%Y %H:%i:%s') as fecha,tr.estado,SUM(dtr.cantidad) as total_cantidad FROM transferencias tr LEFT JOIN almacen loo ON tr.origen = loo.idalmacen LEFT JOIN almacen lod ON tr.destino = lod.idalmacen LEFT JOIN detalle_transferencia dtr ON tr.idtransferencia = dtr.idtransferencia LEFT JOIN usuario u ON tr.idusuario=u.idusuario WHERE tr.origen = '$idalmacenSession' AND DATE(tr.fecha_hora) >= '$fecha_inicio' AND DATE(tr.fecha_hora) <= '$fecha_fin' GROUP BY tr.idtransferencia ORDER BY tr.idtransferencia DESC";
		return ejecutarConsulta($sql);
	}
	...
	{
		$sql = "SELECT 'origen' AS tabla, l.idalmacen AS id, l.ubicacion AS titulo, u.nombre AS usuario, local_ruc AS ruc FROM almacen l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idusuario <> 0 AND l.estado='activado' AND l.eliminado='0'
				UNION ALL
				SELECT 'destino' AS tabla, l.idalmacen AS id, l.ubicacion AS titulo, u.nombre AS usuario, local_ruc AS ruc FROM almacen l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idusuario <> 0 AND l.estado='activado' AND l.eliminado='0'";
	...
	{
		$sql = "SELECT 'origen' AS tabla, l.idalmacen AS id, l.ubicacion AS titulo, u.nombre AS usuario, local_ruc AS ruc FROM almacen l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idalmacen='$idalmacen' AND l.idusuario <> 0 AND l.estado='activado' AND l.eliminado='0'
				UNION ALL
				SELECT 'destino' AS tabla, l.idalmacen AS id, l.ubicacion AS titulo, u.nombre AS usuario, local_ruc AS ruc FROM almacen l LEFT JOIN usuario u ON l.idusuario = u.idusuario WHERE l.idalmacen='$idalmacen' AND l.idusuario <> 0 AND l.estado='activado' AND l.eliminado='0'";
