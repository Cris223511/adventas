...
function init() {
	...
	$.post("../ajax/servicio.php?op=listarTodosActivos", function (data) {
		console.log(data)
	...
	tabla = $('#tbllistado').dataTable(
		{
			...
			"initComplete": function () {
				agregarBuscadorColumna(this.api(), 4, "Buscar por código.");
			},
		}).DataTable();
}
...
function eliminar(idservicio) {
	bootbox.confirm("¿Estás seguro de eliminar el servicio?", function (result) {
		if (result) {
			$.post("../ajax/servicio.php?op=eliminar", { idservicio: idservicio }, function (e) {
				bootbox.alert(e);
				setTimeout(() => {
					location.reload();
				}, 1500);
