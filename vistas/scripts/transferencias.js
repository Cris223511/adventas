...
function validarListarArticulos(idalmacen) {
	console.log(idalmacen);
	if (idalmacen != "") {
		listarArticulos(idalmacen);
		listarArticulosCodigoBarra(idalmacen);
...
function listarArticulosCodigoBarra(idalmacen) {
	$.get("../ajax/transferencias.php?op=selectProducto", { idalmacen: idalmacen }, function (r) {
		$("#idproducto").html(r);
		$('#idproducto').selectpicker('refresh');
	});
}
...
function listarArticulos(idalmacen) {
	...
	if (articuloExistente()) {
		...
		$.ajax({
			...
			success: function (e) {
				console.log(e);
				$('#idproducto').prop("disabled", false);
				console.log("Envío esto al servidor =>", e[0].idarticulo, e[0].articulo, e[0].categoria, e[0].marca, e[0].medida, e[0].stock, e[0].stock_minimo, e[0].precio_venta, e[0].codigo_producto, e[0].codigo, e[0].imagen);

				// Resetear el valor del select
				$('#idproducto').val($("#idproducto option:first").val());
				$("#idproducto").selectpicker('refresh');

				agregarDetalle(e[0].idarticulo, e[0].articulo, e[0].categoria, e[0].marca, e[0].medida, e[0].stock, e[0].stock_minimo, e[0].precio_venta, e[0].codigo_producto, e[0].codigo, e[0].imagen);
